Create a single horizontal sprite strip for the Codex app digital pet `bit` in the state `running`.
...
Output exactly 6 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: Bit is a compact pixel-art AI mentor cube, teal screen face, tiny black eyes, friendly assistant energy, designed for a retro AI engineering dungeon game HUD companion..
