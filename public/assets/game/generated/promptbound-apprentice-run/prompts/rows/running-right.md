Create a single horizontal sprite strip for the Codex app digital pet `promptbound-apprentice` in the state `running-right`.
...
Output exactly 8 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: Beginner AI engineering apprentice, side-view pixel platformer hero, compact readable silhouette, small laptop satchel, cyan scarf, orange code gauntlet, retro dungeon game style style preset: pixel-platformer; animation preset: platformer; animations: idle, walk-right, walk-left, jump, attack, hurt.
